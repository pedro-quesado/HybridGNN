# -*- coding: utf-8 -*-
"""
Created on Fri Sep 23 14:44:32 2022
...
url='https://raw.githubusercontent.com/GLambard/Molecules_Dataset_Collection/master/originals/HIV.csv'
df = pd.read_csv(url)


df['HIV_active'].value_counts() #
df['HIV_active'].value_counts().plot.pie(autopct='%.2f')


X= df.drop(['HIV_active'], axis=1)
...
for i in range(len(df)):
  smiles_list.append(df.iloc[i][0])
...
def create_pytorch_geometric_graph_data_list_from_smiles_and_labels(x_smiles, y):
    ...
    data_list = [G_1, G_2, ...] ... a list of torch_geometric.data.Data objects which represent molecular graphs 
    ...
    for (smiles, y_val) in zip(x_smiles, y):
        ...
        # from SMILES to RDKit mol object
...
data_list = create_pytorch_geometric_graph_data_list_from_smiles_and_labels(smiles_list, labels)
...
pos_weight = torch.FloatTensor([13]).to(device)
...
def train(loader):
    ...
    for batch in loader:
      ...
      loss.backward()  # Calculate the loss and gradients
...
#            Give a name to the file 
    checkpoint_dir = "checkpoints/NAME_THIS"  #Give a name to the file 
    model_dir = "checkpoints/NAME_THIS_model" #Give a name to the file 
    
    #name = "results/NAME_THIS_HIV.txt"        #Give a name to the file 
    #file = open(name,"a")
    #file.write("roc_auc: ")
    ...
    save_ckp(checkpoint_gnn, best_epoch, checkpoint_dir, model_dir, "/checkpoints_NAME_THIS_HIV.pt", "/model_NAME_THIS_HIV.pt" )
